...
@Component({
  selector: 'app-admin-stktrns-report',
  templateUrl: './admin-stktrns-report.component.html',
  styleUrl: './admin-stktrns-report.component.css'
})
export class AdminStktrnsReportComponent {
  adStockReportForm:FormGroup;
  fromDate: Date| null;
  toDate: Date|null;
  locId: number|null;
  ouId: number |null;
  ...
  constructor(private fb: FormBuilder, private router: Router, private service: AdminReportsService, private location1: Location, private router1: ActivatedRoute) {
    this.adStockReportForm = this.fb.group({
    fromDate: [],
      toDate: [],
      locId: [],
      ouId: [],
  ...
  get f() { return this.adStockReportForm.controls; }
  adstkReport(adStockReportForm: any) {
  }
  public validation(){
    this.closeResetButton = false;
    this.progress = 0;
    var locId = this.adStockReportForm.get('locId')?.value;
  ...
  reportDetails() {
    ...
    var pucDt1 = this.adStockReportForm.get('fromDate')?.value;
    var fromDate = this.pipe.transform(pucDt1, 'dd-MMM-yyyy');
    var pucDt2 = this.adStockReportForm.get('toDate')?.value;
    var toDate = this.pipe.transform(pucDt2, 'dd-MMM-yyyy');
    const fileName = 'ADMIN STOCK REPORT OF-' + fromDate + '-TO-' + toDate + '.xlsx';
    var locId = this.adStockReportForm.get('locId')?.value;
    if (locId === null) { locId = undefined }

    const EXT = fileName.substr(fileName.lastIndexOf('.') + 1);
    this.service.AdminStockReport(fromDate, toDate, sessionStorage.getItem('ouId'),locId)
      .subscribe(data => {
